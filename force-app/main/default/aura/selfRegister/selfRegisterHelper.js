({
    ...
    handleSelfRegister: function (component, event, helpler) {
        ...
        var toggleView = component.find("client_error");
        
        startUrl = decodeURIComponent(startUrl);
        var patt = new RegExp("@(student.)?monash(college)?.edu([\\S]+)?$");
        
        if ( patt.test(email) ) {            
            $A.util.removeClass(toggleView, "hide");            
            return;
        }
        $A.util.addClass(toggleView, "hide");        
