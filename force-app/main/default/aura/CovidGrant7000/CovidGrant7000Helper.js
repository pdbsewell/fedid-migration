({
    ...
    Save: function(component, event,helper) {
        ...
        //Set Case Origin
        casRecord.Origin = $A.get("$Label.c.CovidGrant7000CaseOrigin");
        ...
        //Set subject
        casRecord.Subject = $A.get("$Label.c.CovidGrantSubject7000");
        ...
        if (amountNeeded == null || amountNeeded == '' || amountNeeded > 7500 || amountNeeded < 1) {
            var errMsg =  $A.get("$Label.c.CovidGrantValidateAmount");
            if (casRecord.Amount__c > 7500 || casRecord.Amount__c < 1) {
                errMsg = $A.get("$Label.c.CovidGrantValidateAmountEntered") + "7500$ only.";
            }
            var toastEvent = $A.get("e.force:showToast");
            toastEvent.setParams({
                "type": "error",
    ...
    getExistingApplications: function(component) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (component.isValid() && state === "SUCCESS") {
                component.set("v.hasExistingApplications", response.getReturnValue().has7000 !== 0);
