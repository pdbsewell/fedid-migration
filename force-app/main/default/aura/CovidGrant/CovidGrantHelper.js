({
    ...
    Save: function (component, event, helper) {
        ...
        if (casRecord.Amount__c == null || casRecord.Amount__c == '' || casRecord.Amount__c > 500 || casRecord.Amount__c < 1) {
            var errMsg = $A.get("$Label.c.CovidGrantValidateAmount");
            if (casRecord.Amount__c > 5000 || casRecord.Amount__c < 1) {
                errMsg = $A.get("$Label.c.CovidGrantValidateAmountEntered") + "500$ only.";
            }
            var toastEvent = $A.get("e.force:showToast");
            toastEvent.setParams({
                "type": "error",
