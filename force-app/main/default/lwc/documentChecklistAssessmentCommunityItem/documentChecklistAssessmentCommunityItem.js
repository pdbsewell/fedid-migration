...
export default class DocumentChecklistAssessmentCommunityItem extends NavigationMixin(LightningElement) {
        ...
        if (result.error) {
            ...
            switch(this.checklistItem.fields.Status__c.value) {
                case 'Requested':
                    ...
                    if(this.applicationStudyType == 'Graduate Research' && !this.checklistItem.fields.Rule_DeveloperName__c.value.includes('Referee_Check')) {
                        const fileSubmittedEvent = new CustomEvent('filesubmitted', {
                            detail: { documentId: this.checklistItem.fields.Id, status:'Requested' },
                            bubbles: true,
                            composed : true
                        });
                        this.dispatchEvent(fileSubmittedEvent);
                    }                    
                ...
                case 'Submitted':
                    ...
                    if(this.applicationStudyType == 'Graduate Research' && !this.checklistItem.fields.Rule_DeveloperName__c.value.includes('Referee_Check')) {
                        const fileSubmittedEvent = new CustomEvent('filesubmitted', {
                            detail: { documentId: this.checklistItem.fields.Id, status:'Submitted' },
                            bubbles: true,
                            composed : true
                        });
                        this.dispatchEvent(fileSubmittedEvent);
    ...
    //will hide the warning icon from checklist if it is referee
    get isChecklistItemGradResearchReferee() {
        return this.applicationStudyType == 'Graduate Research' && this.checklistItem?.fields.Rule_DeveloperName__c.value.includes('Referee_Check') 
    }
