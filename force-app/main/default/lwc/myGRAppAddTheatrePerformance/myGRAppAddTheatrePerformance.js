...
*  @description used to manage the theatre performace section for graduate research applications 
**/
export default class MyGRAppAddTheatrePerformance extends LightningElement {
    @api applicationId
    statusOptions
    state = 'START'
    saveErrors
    ...
    qualIdToDelete
    qualIdToEdit
    recTypeDevName = 'Theatre_Performance'
    ...
    pickListValues({data,error}){
        if(data){
            this.statusOptions = this.processPicklistOptions(data.PERFORMANCE_STATUS)
        }
    }
    ...
    onFieldChange(e) {
        ...
        switch(fieldName) {
            ...
            case "status":
                ...
                break     
            case "role":
                this.contactQualification.Primary_Role__c = e.target.value
                break
            case "source":
                this.contactQualification.Funding_source__c =  e.target.value
                break      
            case "qualityOfWork":
    ...
    getValidationErrors(){
        ...
        var lightningInputIdList = ["organisation", "name", "duration", "role", "source"]
        ...
        if(!this.contactQualification.Quality_of_work__c) {
            validationErrors.push('Quality of production contains an invalid input')
        }
