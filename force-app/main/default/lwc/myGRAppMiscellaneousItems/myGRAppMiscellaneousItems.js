...
*  @description used to manage the misc. items section for graduate research applications 
**/
export default class MyGRAppMiscellaneousItems extends LightningElement {
    @api applicationId
    state = 'START'
    saveErrors
    ...
    qualIdToDelete
    qualIdToEdit
    recTypeDevName = 'Other_Qualification'
    ...
    getValidationErrors(){
        ...
        var lightningInputIdList = ["type", "name"]
        ...
        if(!this.contactQualification.Quality_of_work__c) {
            validationErrors.push('Quality of item contains an invalid input')
        }
    ...
    @api validateDeclaration() {
        ...
        if (this.declarationSigned === true && !this.contactQualifications?.length) {
            let validationMessage = 'Please enter at least one item to proceed'
