...
import getEnglishVerificationCustomSetting from '@salesforce/apex/EnglishTestApiService.getEnglishVerificationCustomSetting';
...
export default class DocumentChecklistAssessmentCommunityList extends LightningElement {
    ...
    @api englishVerificationCustomSetting;

    @wire(getEnglishVerificationCustomSetting)
    wiredEnglishVerificationCustomSetting({ error, data }) {
        if (data) {
            this.englishVerificationCustomSetting = data;
        } else if (error) {
            // Handle error
