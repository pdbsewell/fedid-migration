...
import retrieveApplicationDetailsAndCourseOfferings from '@salesforce/apex/ApplicationCoursePreferenceWizard.retrieveApplicationDetailsAndCourseOfferings';
import searchCourseOfferings from '@salesforce/apex/ApplicationCoursePreferenceWizard.CourseOfferingsSearch';
import addACP from '@salesforce/apex/ApplicationCoursePreferenceWizard.addACP';

export default class AddApplicationCoursePreferenceWizard extends NavigationMixin(LightningElement) {
    @api applicationId;
    ...
    @track latestChanged;
    @track addAcpDisabled;
    @track showConfirmation;
    @track isSubmittingACP;
    @track message;
    @track dockedClass='slds-docked-composer slds-grid slds-grid_vertical slds-is-open';

    get selectedOfferingAttendanceType(){
        return this.convertAttendanceTypeName(this.selectedOffering.Attendance_Type__c);
    }
    ...
    connectedCallback() {
        ...
        .then(result => {
            ...
            this.selectedCitizenshipType = this.application.Residency_Status__c;
            
            if(this.firstAcp){
                //if ACP exists, get details from 1st ACP
                //Set admission year
                ...
                //If no ACP exists, default values for Admission year and Attendance Mode Type
                let currentYear = new Date();
                this.selectedAdmissionYear =  currentYear.getFullYear().toString(10);
                this.selectedAttendanceModeType = 'Full time - On Campus';
            }
            //Reset filter fields
            this.reset();
    ...
    filterCourseOfferings(){
        ...
        thisContent.delayTimeout = setTimeout(function () {
            ...
            let filterOfferings = thisContent.loadedCourseOfferings.filter(function(courseOffering) {
                ...
                //filter by calendar           
                if(courseOffering.Admission_Calendar_Description__c && thisContent.searchFilter.calendar !== ''){
                ...
                if(courseOffering.Admission_Category__c && thisContent.searchFilter.admissionCategory !== ''){
                ...
                //filter by location
                if(courseOffering.Location_Code__c && thisContent.searchFilter.location !== ''){
                ...
                if(thisContent.searchFilter.unitSet !== ''){
                    ...
                    if(courseOffering.Unit_Set_Code__c === undefined){
                        ...
                        if(courseOffering.Unit_Set_Code__c && courseOffering.Unit_Set_Description__c){
                ...
                //filter by version
                if(courseOffering.Unit_Set_Version__c && thisContent.searchFilter.version !== ''){
                    //default to not matched if value undefined
    ...
    get hasSearchResult() {
        return this.filteredCourseOfferings.length > 0;
    }
        ...
        if(event.detail.parameters.selected){
            ...
            this.template.querySelectorAll("c-add-application-course-preference-wizard-result").forEach(function(element) {  
                if(element.courseOffering.Id !== event.detail.parameters.courseOfferingId){
                    ...
                    if(thisContent.selectedOffering.Admission_Calendar_Description__c){
                        calendarDescription = thisContent.selectedOffering.Admission_Calendar_Description__c.split(' ');
    ...
    addACP(){
        ...
        addACP({
            applicationId : this.applicationId,
            selectedCourseOfferingId : this.selectedOffering.Id
        })
        .then(result => {
            if(result.preference){
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Application Course Preference added',
                        message: 'Successfully added the Application Course Preference as the student\'s preference number: ' + result.preference + '.',
                        variant: 'success'
                    })
                );            
            }else{
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error on adding Application Course Preference',
                        message: 'There was an issue adding the new application course preference.',
                        variant: 'error'
                    })
                );    
            }
            ...
            if(result.preference){
                //redirect to the acp detail page
                this[NavigationMixin.Navigate]({
                    type: 'standard__recordPage',
                    attributes: {
                        recordId: result.acpId,
                        objectApiName: 'Application_Course_Preference__c',
                        actionName: 'view'
                    }
                });
            }
    ...
    showDockedPanel(){
        this.dockedClass = 'slds-docked-composer slds-grid slds-grid_vertical slds-is-open';
    }

    hideDockedPanel(){
        this.dockedClass = 'slds-docked-composer slds-grid slds-grid_vertical slds-is-closed';
    }
}