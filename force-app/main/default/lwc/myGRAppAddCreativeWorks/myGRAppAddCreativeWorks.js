...
*  @description used to manage the creative works for graduate research applications 
**/
export default class MyGRAppAddCreativeWorks extends LightningElement {
    
    @api applicationId
    languageOptions
    typeOfPublisherOptions
    ...
    pickListValues({data,error}){
        if(data){
            ...
            this.typeOfPublisherOptions = this.processPicklistOptions(data.PUBLISHER_TYPE)
            this.publicationStatusOptions = this.processPicklistOptions(data.CREATIVE_WORK_STATUS)
        }
    }
    ...
    onFieldChange(e) {
        ...
        switch(fieldName) {
            case "typeOfPublisher":
                this.contactQualification.Type_of_publisher__c = e.target.value
                break
            ...
            case "title":
                ...
                break
            case "wordLengthOfWork":
                this.contactQualification.Word_Length__c = Number(e.target.value)
                break
            case "status":
    ...
    getValidationErrors(){
        ...
        var lightningSelect = ["typeOfPublisher","status","numberOfAuthors","contributionPercentage","nonEnglishLanguages"]
        ...
        var lightningInputIdList = ["name","title","wordLengthOfWork"]
        ...
        var lightningTextAreaList = ["qualityOfWork"]
        ...
        if(!this.contactQualification.Description__c) {
            validationErrors.push('Describe your contribution contains an invalid input')
        }
    ...
    @api validateDeclaration() {
        ...
        if (this.declarationSigned === true && !this.contactQualifications?.length) {
            let validationMessage = 'Please enter at least one creative work to proceed'
